...
/**
 * A building template gives the shape of the building, and is used to place a building onto the game board. To place a building we need to give it an origin and a direction (North, South, East, West)
 * 
...
public abstract class BuildingTemplate {
	...
	/**
	 * Get the name of this building type. For example, "CornerBuilding", "LargeBuilding", etc.
	 * 
	 * @return
	 */
	 ...
	 * 
	 * @param origin
	 *            The origin where the building is placed
	 * @param direction
	 *            The direction in which the building will be facing
	 * @param player
	 *            The player placing the building
	 */
	public Building create(Position origin, Game.Direction direction, Game.Player player) {
	...
	/**
	 * Rotate an array of squares to face in a given direction. The squares are assumed to be facing initially in the NORTH direction.
	 * 
	 * @param squares
	 * @param direction
	 */
	...
	/**
	 * Translate (or shift) the squares by a given x and y component. For example:
	 * 
	 * <pre>
	 *  1 |_|X|_|_|
	 *  0 |X|X|X|_|
	 ...
	 * </pre>
	 * 
	 * If we translate by (1,1), which corresponds to one step in the X direction and one step in the Y direction.
	 * 
	 * @param squares
	 * @return
	 */
	private static Position[] translate(Position[] squares, Position translation) {
		...
		for (int i = 0; i != squares.length; ++i) {
			Position square = squares[i];
			nSquares[i] = new Position(square.getX() + translation.getX(), square.getY() + translation.getY());
		}
		return nSquares;
	}
}
