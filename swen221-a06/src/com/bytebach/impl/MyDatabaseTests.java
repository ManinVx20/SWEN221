...
//	}
	...
	public static void checkTable(Database db, String table, Value[][] values) {
		...
		for (int i = 0; i != values.length; ++i) {
			...
			if (dbrow.size() != valuerow.length) {
				fail("Table " + table + ", row " + i + " has " + dbrow.size() + " entries, should have " + valuerow.length);
			}
			for (int j = 0; j != dbrow.size(); ++j) {
				Value dbv = dbrow.get(j);
				Value ddbv = valuerow[j];
				if (dbv == null || !dbv.equals(ddbv)) {
					fail("Table " + table + ", row " + i + ", entry " + j + " is " + dbv + ", should be " + ddbv);
				}
			}
		}
	}
	...
	public static void removeRow2(Database db, String table, Object... keys) {
		...
		// passing only the row number as Integer[]
	...
	public static void modifyRow1(Database db, String table, int index, int entry, Object value) {
		System.out.println("> MODIFYING ROW AT INDEX " + index + ", " + entry + " IN \"" + table + "\" : " + value);
	...
	/**
	 * Convert a 2d array of objects into a 2d array of Value objects. The reason for this method is that it simplifies the construction of test databases.
	 * 
	 * @param rawData
	 * @return
	 */
	...
	/**
	 * Convert a 1d array of objects into a 1d array of Value objects. The reason for this method is that it simplifies the construction of test databases.
	 * 
	 * @param rawData
	 * @return
	 */
