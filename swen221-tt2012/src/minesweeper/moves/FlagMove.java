	...
	public void apply(Game game) throws SyntaxError {
		...
		if(!sq.isHidden()) {
			throw new SyntaxError(
					"invalid move " + this + " --- cannot flag/unflag square which is already exposed");	
		}
		
		if(sq.isFlagged() && isFlagged) {
			throw new SyntaxError(
					"invalid move " + this + " --- cannot flag square which is already flagged");	
		}
		
		if(!sq.isFlagged() && !isFlagged) {
			throw new SyntaxError(
					"invalid move " + this + " --- cannot unflag square which is already unflagged");
		}
