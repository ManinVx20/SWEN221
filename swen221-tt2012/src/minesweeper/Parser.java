...
public class Parser {
	...
	/**
	 * Parse a position which is a string of the form "(x,y)" where x and y are integers giving the x and y components.
	 * 
	 * @return
	 * @throws SyntaxError
	 */
	private Position parsePosition(int width, int height) throws SyntaxError {
		...
		int x = parseNumber();
		if (x<0 || x>=width){
			syntaxError("Position exceed board width");
		}
		match(',');
		int y = parseNumber();
		if (y<0 || y>=height){
			syntaxError("Position exceed board heig");
		}
		match(')');
		// TODO: check that x and y are a valid position on the board.
		return new Position(x, y);
	}

	/**
	 * Parse a move which consists of special character indicating what kind of move, followed by a position string. The move characters are: 'E' for expose move; 'F' for a flag move; 'U' for an
	 * unflag move; 'W' for a winning move; 'L' for a losing move which exposes a bomb.
	 * 
	 * @return
	 * @throws SyntaxError
	 */
	...
	/**
	 * This method checks whether the given character is at the current position in the text. The method will first skip any whitespace at this point.
	 * 
	 * @param text
	 */
	...
	/**
	 * Move the current position past any "white space" characters. White space characters include spaces (i.e. ' '), tabs (i.e. '\t') and newlines (i.e. '\n').
	 */
