...
public class Trick {
	...
	public Player.Direction getWinner() {
		...
		for (int i = 0; i != 4; ++i) {
			if (cards[i].suit().equals(winningCard.suit())
	...
	public void play(Player p, Card c) throws IllegalMove {
		...
		// First, check that this player is entitled to play now.		
		if (getNextToPlay() != p.getDirection()) {
			throw new IllegalMove("Invalid player to move");
		}
		
		// Second, check that the card being played is in the players hand.
		if(!p.getHand().contains(c)) {
			throw new IllegalMove("Player attempting to play card not in his hand!");
		}		
		
		// Third, check that this player has followed suit.		
		Card leadCard = getCardPlayed(getLeadPlayer());

		if (leadCard != null && !c.suit().equals(leadCard.suit())
				&& p.getHand().matches(leadCard.suit()).size() > 0) {
			throw new IllegalMove("Card does not follow suit: (" + c + ")");
		}
