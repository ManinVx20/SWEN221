 ...
 */
public class Canvas {
	public final Color DEFAULT_CANVAS_COLOR = Color.WHITE;
	private int width;
	private int height;
	private Color[][] grid;
			...
			for(int i=0;i!=nwidth;++i) {
				for(int j=0;j!=nheight;++j) {
					if(i < width && j < height) {
						...
						ngrid[i][j] = DEFAULT_CANVAS_COLOR;
					}
				}	
			}
	...
	public void show() {		
		...
		JFrame window = new JFrame("Assignment 4, Canvas Viewer");
		...
		window.setPreferredSize(new Dimension(width+20, height+40));
